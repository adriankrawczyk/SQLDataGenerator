...
def generate_students():
    with open(students_sql_path, "w", encoding="utf-8") as students_file:
        ...
        for country_name, places in countries.items():
            ...
            if city_id is None:
                continue  # Skip if country not in our defined list
            
            # Skip if no names available for this country
            if country_name not in men_first_names or country_name not in women_first_names or \
               country_name not in men_last_names or country_name not in women_last_names:
                continue
